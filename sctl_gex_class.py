    ...
    def PP(self):
        ...
        if self.rename_cluster==True:
            ...
            if cluster_numbers_len==len(self.new_cluster_names):
                self.adata.obs['Cell_Clusters_Named']=self.adata.obs['leiden']
                self.adata.rename_categories('Cell_Clusters_Named', self.new_cluster_names)
        ...
        if self.umap_marker_gene==True:
            ...
            if ((self.rename_cluster==True) & (cluster_numbers_len==len(self.new_cluster_names))):
                ...
                sc.pl.umap(self.adata, color=self.umap_marker_gene_list+['leiden','Cell_Clusters_Named'],save=self.output_prefix+'umap_markergenes.pdf')
            else:
